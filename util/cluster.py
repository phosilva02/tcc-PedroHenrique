...
class Cluster:
    ...
    def __init__(self, label, parent, birthLevel, numPoints, knnGraph):
        self.label = label
        self.birthLevel = birthLevel
        self.deathLevel = 0
        self.numPoints = numPoints
        self.knnGraph = knnGraph
        
        # store unsupervised measures information
        self.metric = "modQ"
        self.stability = 0.0
        self.propagatedStability = 0
        self.modQ = 0.0
        self.propagatedModQ = 0
    ...
    def detachPoints(self, numPoints, level):
        ...
        #print(f"Objetos no cluster {self.label} = {self.objects}")
        self.modQ = self.knnGraph.calculateModQ(self.objects)
    ...
    def propagate(self):
        if(self.metric == "stability"):
            self.propagateStability()
        if(self.metric == "modQ"):
            self.propagateModQ()

    def propagateModQ(self):
        if self.parent != None:
            ...
            if not self.hasChildren:
                self.parent.propagatedModQ = self.modQ
                self.parent.propagatedDescendants.append(self)
            
            else:
                if self.modQ >= self.propagatedModQ:
                    self.parent.propagatedModQ = self.modQ
                    self.parent.propagatedDescendants.append(self)
                else:
                    self.parent.propagatedModQ = self.propagatedModQ
                    self.parent.propagatedDescendants = (self.parent.propagatedDescendants + self.propagatedDescendants)

    def propagateStability(self):
