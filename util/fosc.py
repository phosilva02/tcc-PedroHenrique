...
class FOSC:
    ...
    def __init__(self, Z, mClSize=4, filename=None):
        ...
        matrix = np.genfromtxt(filename, dtype=float, delimiter=';', missing_values=np.nan)
        ...
        self.clusters.append(Cluster(1, None, np.NaN, self.numObjects, self.knn))

        #print(f"Objetos no clustes: {self.clusters[1].getObjects()}")
    ...
    def _createNewCluster(self, points, clusterLabels, parentCluster, clusterLabel, levelWeight):
        ...
        if clusterLabel != 0:
            cluster = Cluster(clusterLabel, parentCluster, levelWeight, len(points), self.knn)
            cluster.setObjects(points)
            #cluster.setStability(self.knn.calculateModQ(cluster.getObjects()))
            #print(f"Cluster {cluster.getLabel()} = {cluster.getStability()}")
            return cluster
        else:
    ...
    def findProminentClusters(self, rootTree, infiniteStability):
        ...
        for cluster in solution:
            ...
            #print(f"Cluster {cluster.getLabel()} objects = {cluster.getObjects()}")
